...
let
  ...
  mkEmailAccountOpts = { description }: {
    enable = mkOption {
      type = types.bool;
      default = false;
      description = "Enable ${description} email account";
    };

    address = mkOption {
      type = types.str;
      default = "";
      description = "Primary email address for ${description}";
    };

    aliases = mkOption {
      type = types.listOf types.str;
      default = [];
      description = "Email aliases for ${description}";
    };

    realName = mkOption {
      type = types.str;
      default = "";
      description = "Display name for ${description}";
    };

    passwordCommand = mkOption {
      type = types.str;
      default = "";
      description = "Command to retrieve the account password";
    ...
    (mkIf protonCfg.enable {
      accounts.email.accounts.proton = {
        ...
        aerc = {
          ...
          smtpAuth = "login";
          extraAccounts = {
            source = "imap+insecure://${protonCfg.address}@127.0.0.1:1143";
            outgoing = "smtp+insecure+login://${protonCfg.address}@127.0.0.1:1025";
          };
        };

        himalaya = mkIf himalayaEnabled {
          enable = true;
        };
      };
    })
