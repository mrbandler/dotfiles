{ den, inputs, ... }: {
  den.aspects.desktop-niri = {
    includes = [ den.aspects.desktop-keybindings ];
          ...
          settings = {
            ...
            workspaces = builtins.listToAttrs (
              map (ws: {
                name = ws.name;
                value = {
                  name = if ws.displayName != null then ws.displayName else ws.name;
            ...
            input = {
              keyboard = {
                xkb = { layout = "us,de"; options = "grp:win_space_toggle"; };
                repeat-delay = 600;
                repeat-rate = 25;
                track-layout = "global";
              };
              touchpad = {
                tap = true; dwt = true; natural-scroll = true;
                accel-speed = 0.0; accel-profile = "adaptive";
              };
              mouse = {
                accel-speed = 0.0; accel-profile = "adaptive"; natural-scroll = false;
              };
            };
            ...
            layout = {
              ...
              preset-column-widths = [
                { proportion = 0.33333; } { proportion = 0.5; }
                { proportion = 0.66667; } { proportion = 1.0; }
              ];
              ...
              focus-ring = {
                ...
                active.gradient = {
                  ...
                  angle = 90; relative-to = "workspace-view";
                };
              ...
              shadow = {
                enable = true; softness = 30; spread = 5;
                offset = { x = 0; y = 5; }; color = "#0007";
              };
            ...
            window-rules = [
              {
                ...
                geometry-corner-radius = {
                  top-left = 5.0; top-right = 5.0;
                  bottom-left = 5.0; bottom-right = 5.0;
                };
              }
            ...
            binds = mkIf cfg.enable (
              let
                nav = cfg.navigation; lay = cfg.layout;
                win = cfg.window; app = cfg.applications; mon = cfg.monitor;
              in
              {
                ...
                "${win.overview}" = { repeat = false; action.toggle-overview = {}; };
                "${win.inhibitShortcuts}" = { allow-inhibiting = false; action.toggle-keyboard-shortcuts-inhibit = {}; };
              }
              // (builtins.listToAttrs (map (key: {
                name = key; value = { repeat = false; action.close-window = {}; };
              }) win.close))
              ...
              // {
                "Super+WheelScrollUp" = { cooldown-ms = 150; action.focus-workspace-up = {}; };
                "Super+WheelScrollDown" = { cooldown-ms = 150; action.focus-workspace-down = {}; };
              }
              ...
              // {
                ${cfg.media.volumeUp} = { allow-when-locked = true; action.spawn = cmds.media.volumeUp; };
                ${cfg.media.volumeDown} = { allow-when-locked = true; action.spawn = cmds.media.volumeDown; };
                ${cfg.media.mute} = { allow-when-locked = true; action.spawn = cmds.media.mute; };
                ${cfg.media.micMute} = { allow-when-locked = true; action.spawn = cmds.media.micMute; };
                ${cfg.media.play} = { allow-when-locked = true; action.spawn = cmds.media.play; };
                ${cfg.media.stop} = { allow-when-locked = true; action.spawn = cmds.media.stop; };
                ${cfg.media.prev} = { allow-when-locked = true; action.spawn = cmds.media.prev; };
                ${cfg.media.next} = { allow-when-locked = true; action.spawn = cmds.media.next; };
              }
              ...
              // (builtins.listToAttrs (map (binding: {
                name = binding.key;
                value.action.${binding.action} = if binding.args == [] then {} else binding.args;
              }) cfg.extraNiri))
            );
          };
        };
